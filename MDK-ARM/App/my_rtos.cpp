...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  if (GPIO_Pin==g_xyPlatform.x->limit_switch1_pin)
  {
    ...
    {
      if (g_xyPlatform.IsHoming() && g_xyPlatform.x->mode != x_linear_module::MODULE_MODE_POSITION)
      {
      ...
      else if (!g_xyPlatform.IsHoming())
      {
        g_xyPlatform.ReportSafetyError(xy_platform::PLATFORM_ERROR_NEGATIVE_LIMIT);
      }
    }
  }
  else if (GPIO_Pin == g_xyPlatform.x->limit_switch2_pin)
  {
    ...
    {
      g_xyPlatform.ReportSafetyError(xy_platform::PLATFORM_ERROR_POSITIVE_LIMIT);
    }
  }
  else if (GPIO_Pin == g_xyPlatform.y->limit_switch1_pin)
  {
    ...
    {
      //y轴回零边界
      if (g_xyPlatform.IsHoming() && g_xyPlatform.y->mode != x_linear_module::MODULE_MODE_POSITION)
      {
...
void StartDefaultTask(void *argument)
{
  ...
  g_xyPlatform.ConfigureWorkspace(0.0f, 300.0f, 0.0f, 300.0f);
