...
typedef enum {
  PLATFORM_ERROR_NONE = 0x00,
  PLATFORM_ERROR_NOT_HOMED = 0x01,
  PLATFORM_ERROR_OUT_OF_RANGE = 0x02,
  PLATFORM_ERROR_NEGATIVE_LIMIT = 0x03,
  PLATFORM_ERROR_POSITIVE_LIMIT = 0x04,
} PlatformError_t;
...
public:
  ...
  float y_max = 300.0f;
  bool homed = false;
  uint8_t error_code = PLATFORM_ERROR_NONE;
  ...
  void ConfigureWorkspace(float x_min, float x_max, float y_min, float y_max);
  ...
  void ReportSafetyError(uint8_t error_code);

  uint8_t GetErrorCode(void) const;

  void SetCurrentAsZero(void);
