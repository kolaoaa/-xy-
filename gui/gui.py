...
# On Windows, importing Qt widgets before PyTorch can make torch\lib\c10.dll
# fail to initialize. Preload the runtime while the DLL search state is clean.
...
from camera_calibration import Bounds, load_sorting_config
from camera_ownership import CameraOwnershipCoordinator
from gripper_debug_panel import GripperDebugPanel
...
class XYPlotCanvas(FigureCanvas):
    ...
    def __init__(self, parent=None, width=6, height=5, dpi=100):
        ...
        self.planned_path_line, = self.ax.plot([], [], '--', color='tab:orange', linewidth=1.5, label='Planned')
        self.detected_points = self.ax.scatter([], [], marker='x', color='tab:purple', s=45, label='Detected')
        self.bin_points = self.ax.scatter([], [], marker='s', color='tab:brown', s=55, label='Bins')
        self.active_target_point, = self.ax.plot([], [], '*', color='tab:red', markersize=14, label='Pick')
    ...
    def set_sorting_overlay(self, detections=None, planned_path=None, bins=None, current_target=None):
        """Update planned path, detected objects, bin points, and active target."""

        if planned_path is not None:
            if planned_path:
                xs, ys = zip(*planned_path)
                self.planned_path_line.set_data(list(xs), list(ys))
            else:
                self.planned_path_line.set_data([], [])
        ...
        if detections is not None:
            ...
            for item in detections:
                if isinstance(item, dict):
                    ...
                    x_value = getattr(item, "platform_x_mm", None)
                    y_value = getattr(item, "platform_y_mm", None)
                if x_value is not None and y_value is not None:
                    offsets.append([float(x_value), float(y_value)])
            self.detected_points.set_offsets(np.asarray(offsets, dtype=float) if offsets else np.empty((0, 2)))

        if bins is not None:
            offsets = [[float(point[0]), float(point[1])] for point in bins.values()]
            self.bin_points.set_offsets(np.asarray(offsets, dtype=float) if offsets else np.empty((0, 2)))

        if current_target is not None:
            self.active_target_point.set_data([float(current_target[0])], [float(current_target[1])])
        else:
            self.active_target_point.set_data([], [])
...
class XYPlatformController:
    ...
    def __init__(self, communicator: USBCommunicator, signal_emitter: SignalEmitter):
        ...
        self.platform_bounds = Bounds(0.0, 300.0, 0.0, 300.0)

    def set_platform_bounds(self, bounds: Bounds):
        self.platform_bounds = bounds
    
    def send_command(self, cmd_bytes: bytes, cmd_name: str = "", silent: bool = False):
        """发送命令"""
    ...
    def home(self):
        """回零"""
        success = self.send_command(CommandBuilder.home(), "回零")
        if success:
            self.zeroed = False
            self.home_pending = True
            self._mark_status(PlatformStatus.HOMING)
        return success

    def set_zero(self):
        """将当前位置设为软件零点"""
        success = self.send_command(CommandBuilder.set_zero(), "手动设零")
        if success:
            self.current_x = 0.0
            self.current_y = 0.0
            self.current_status = PlatformStatus.IDLE
            ...
            self.zeroed = True
            self.home_pending = False
            self.signal.status_updated.emit(
                {
                    "x": self.current_x,
                    "y": self.current_y,
    ...
    def move_abs(self, x: float, y: float, speed: int):
        """绝对位移"""
        if not self.platform_bounds.contains(x, y):
            self.signal.error_occurred.emit(f"目标越界: ({x:.1f}, {y:.1f})")
            return False
        ...
        if success:
            self._mark_status(PlatformStatus.MANUAL)
        return success
    ...
    def move_rel(self, dx: float, dy: float, speed: int):
        ...
        if not self.platform_bounds.contains(target_x, target_y):
            self.signal.error_occurred.emit(f"目标越界: ({target_x:.1f}, {target_y:.1f})")
            return False
        success = self.send_command(
    ...
    def line_interp(self, x1: float, y1: float, x2: float, y2: float, speed: int):
        ...
        if success:
            self._mark_status(PlatformStatus.INTERPING)
        return success
    ...
    def servo_open(self):
        """张开夹子"""
        return self.send_command(CommandBuilder.servo_open(), "夹子张开")

    def servo_close(self):
        """闭合夹子"""
        return self.send_command(CommandBuilder.servo_close(), "夹子闭合")

    def servo_middle(self):
        """夹子回到中位"""
        return self.send_command(CommandBuilder.servo_middle(), "夹子中位")
    
    def query_status(self, silent: bool = False):
        """查询状态"""
        return self.send_command(CommandBuilder.query_status(), "查询状态", silent=silent)

    def is_idle(self):
        return self.current_status == PlatformStatus.IDLE

    def current_position(self):
        return self.current_x, self.current_y

    def _mark_status(self, status: PlatformStatus):
    ...
    def handle_response(self, frame_data: bytes):
        ...
        try:
            ...
            if cmd == CommandType.STATUS_RESPONSE:
                ...
                if self.current_status == PlatformStatus.ERROR or self.error_code != 0:
                    self.zeroed = False
                    self.home_pending = False
                elif self.home_pending:
                    self.zeroed = self.current_status == PlatformStatus.IDLE
                    if self.zeroed:
                        self.home_pending = False
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.sorting_config = load_sorting_config(self.gui_dir / "config" / "sorting_config.json")
        self.camera_ownership = CameraOwnershipCoordinator()
        ...
        self.controller.set_platform_bounds(self.sorting_config.platform_bounds)
    ...
    def create_basic_control_group(self) -> QGroupBox:
        ...
        set_zero_btn = QPushButton("手动设零")
        set_zero_btn.setMinimumHeight(40)
        set_zero_btn.setFont(QFont("Arial", 11, QFont.Bold))
        set_zero_btn.clicked.connect(self.controller.set_zero)
        layout.addWidget(set_zero_btn)
    ...
    def create_positioning_group(self) -> QGroupBox:
        ...
        self.abs_x_input.setRange(self.sorting_config.platform_bounds.x_min, self.sorting_config.platform_bounds.x_max)
        self.abs_x_input.setValue(0)
        layout.addWidget(self.abs_x_input, 2, 0, 1, 2)
        
        self.abs_y_input = QDoubleSpinBox()
        self.abs_y_input.setRange(self.sorting_config.platform_bounds.y_min, self.sorting_config.platform_bounds.y_max)
    ...
    def create_plot_and_log_panel(self) -> QWidget:
        ...
        self.acceptance_panel = AcceptancePanel(
            self.sorting_config,
            self.camera_ownership,
            parent=self,
        )
        self.gripper_debug_panel = GripperDebugPanel(self.controller, self)
        tabs = QTabWidget()
        tabs.addTab(self.sorting_panel, "视觉分类")
        tabs.addTab(self.acceptance_panel, "识别验收")
        tabs.addTab(self.gripper_debug_panel, "调试夹子")
        ...
        motion_layout.addWidget(plot_group, 2)
        ...
        motion_layout.addWidget(log_group, 1)

        motion_widget.setLayout(motion_layout)
        tabs.addTab(motion_widget, "轨迹与日志")
    ...
    def on_status_updated(self, status_info: dict):
        ...
        if hasattr(self, "sorting_panel"):
            self.sorting_panel.on_platform_status(status_info)
    ...
    def update_sorting_overlay(self, detections=None, planned_path=None, bins=None, current_target=None):
        if hasattr(self, "canvas"):
            self.canvas.set_sorting_overlay(
    ...
    def closeEvent(self, event):
        """关闭事件"""
        if hasattr(self, "sorting_panel"):
            self.sorting_panel.shutdown()
        if hasattr(self, "acceptance_panel"):
            self.acceptance_panel.shutdown()
        if self.comm.is_connected():
            self.comm.disconnect()
        event.accept()
