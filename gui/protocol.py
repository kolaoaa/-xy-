...
class CommandType(IntEnum):
    ...
    SET_ZERO = 0x08            # 手动设零
    SERVO_OPEN = 0x09          # 夹子张开
    SERVO_CLOSE = 0x0A         # 夹子闭合
    SERVO_MIDDLE = 0x0B        # 夹子中位
...
class CommandBuilder:
    ...
    def set_zero() -> bytes:
        """构建手动设零命令"""
        return ProtocolFrame().pack(CommandType.SET_ZERO, b'')
    ...
    @staticmethod
    def servo_open() -> bytes:
        """构建夹子张开命令"""
        return ProtocolFrame().pack(CommandType.SERVO_OPEN, b'')

    @staticmethod
    def servo_close() -> bytes:
        """构建夹子闭合命令"""
        return ProtocolFrame().pack(CommandType.SERVO_CLOSE, b'')

    @staticmethod
    def servo_middle() -> bytes:
        """构建夹子中位命令"""
        return ProtocolFrame().pack(CommandType.SERVO_MIDDLE, b'')
